...
from PySide6.QtCore import QAbstractTableModel, QModelIndex, QPersistentModelIndex, Qt, QStandardPaths, QAbstractListModel, Signal
...
# from packy.core.app_settings import PreferencesTask, PreferencesKeys, QObject, AppSettings
from typing import override

from packy.graveyard.task_model import TaskModel
...
###############################################################################
class FinalMeta(type(QAbstractTableModel), type(Configurable)):  # pyright: ignore[reportGeneralTypeIssues]  # noqa: D101
    pass

###############################################################################
@final
class TaskListModel(QAbstractTableModel, Configurable, metaclass=FinalMeta):
    ...
    def __init__(self, parent: QObject | None = None) -> None:
        ...
        self.__tasks: dict[int,TaskModel] = {}
        # self.__u_dash = "\u2014"
        # self.__u_check = "\u2713"
        # self.__u_cross = "\u2715"
    ...
    def columnCount(self, parent: QModelIndex | QPersistentModelIndex | None = None) -> int:
        ...
        if parent.isValid():
            return 0
        return len(TaskListModel.Column)
    ...
    def headerData(  # noqa: PLR0911
        ...
        if role == Qt.ItemDataRole.DisplayRole and orientation == Qt.Orientation.Horizontal:
            match TaskListModel.Column(section):
                case TaskListModel.Column.STATUS:
                    return self.tr("Status")
                case TaskListModel.Column.SRC_FOLDER:
                    return self.tr("Source Folder")
                case TaskListModel.Column.DST_BASENAME:
                    return self.tr("Destination Base Name")
                case TaskListModel.Column.DST_FILE:
                    return self.tr("Destination File")
    ...
    def data(
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            match TaskListModel.Column(column):
                case TaskListModel.Column.STATUS:
                    return task.is_enabled
                case TaskListModel.Column.SRC_FOLDER:
                    return task.source_folder
                case TaskListModel.Column.DST_BASENAME:
                    return "" if task.destination_folder is None else task.destination_folder.parent
                case TaskListModel.Column.DST_FILE:
                    return "" if task.output_filename is None else task.output_filename
        return None
    ...
    def setData(
        ...
        if role == Qt.ItemDataRole.EditRole:
            match TaskListModel.Column(column):
                case TaskListModel.Column.STATUS:
                    task.enable() if value is True else task.disable()
                case TaskListModel.Column.SRC_FOLDER:
                    task.set_source_folder(value)
                case TaskListModel.Column.DST_BASENAME:
                    task.set_destination_folder(value)
                case TaskListModel.Column.DST_FILE:
                    task.set_output_filename(value)
                case _:
                    return False
    ...
    def add_task(self) -> None:
        # TODO voir LayerModel::insertLayer
    ...
    #     if not hasattr(TaskListModel, "funcDstSuffix"):
    ...
    #     dst_suffix = TaskListModel.funcDstSuffix(self)
    ...
    #     func_dst_suffix = TaskListModel.__suffixTimeStamp

    #     match value:
    #         case PreferencesTask.SUFFIX_CURR_DATE:
    #             func_dst_suffix = TaskListModel.__suffixTimeStamp
    #         case PreferencesTask.SUFFIX_VERSION_NUM:
    #             func_dst_suffix = TaskListModel.__suffixId
    #         case PreferencesTask.SUFFIX_NOTHING:
    #             func_dst_suffix = TaskListModel.__suffixNothing
    #         case _:
    #             msg = "value not recognized."
    #             QtCore.qWarning(msg)

    #     TaskListModel.funcDstSuffix = func_dst_suffix
